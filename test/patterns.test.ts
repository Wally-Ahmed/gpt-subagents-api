...
describe("parseFrontmatter — close-delimiter bug regression", () => {
  it("does NOT treat `---not-a-close` as the closing delimiter", () => {
    ...
    const { meta, body } = parseFrontmatter(raw);
    // No standalone `---` close exists, so there is no frontmatter at all:
    // the whole thing is body and `name` is NOT extracted.
  ...
  it("uses the first standalone `---` line as the close, not a `---x` line", () => {
    ...
    const { meta, body } = parseFrontmatter(raw);
    // The frontmatter block now spans up to the standalone `---`; the
    // `---trailing` line is treated as a metadata line (no colon -> skipped).
    expect(meta.name).toBe("demo");
...
// GPT_SUBAGENTS_PATTERNS_DIR override, mirroring how the sibling repo redirects
// its token dir for tests. The module is re-imported fresh so it re-reads the
// env-driven PATTERNS_DIR.
describe("pattern catalog (temp patterns dir)", () => {
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "gpt-subagents-patterns-"));
    process.env.GPT_SUBAGENTS_PATTERNS_DIR = dir;
    vi.resetModules(); // so patterns.ts re-reads PATTERNS_DIR on next import
  });

  afterEach(() => {
    rmSync(dir, { recursive: true, force: true });
    delete process.env.GPT_SUBAGENTS_PATTERNS_DIR;
    vi.resetModules();
  });
  ...
  it("returns an empty catalog when the dir does not exist", async () => {
    process.env.GPT_SUBAGENTS_PATTERNS_DIR = join(dir, "nope");
    vi.resetModules();
    const m = await freshPatterns();
  ...
  it("a file whose close delimiter is `---not-a-close` is not cataloged", async () => {
    // The name lives inside what looks like frontmatter but has no standalone
    // `---` close, so parseFrontmatter yields no `name` and it is excluded.
    writePattern("bad.md", "---\nname: sneaky\n---not-a-close\nbody");
    const m = await freshPatterns();
    expect(m.patternNames()).toEqual([]);
  });
